...
class MyEnv:
    ...
    def reset(self, seed = None):
        ...
        q_zero,ik_err_stack,ik_info = solve_ik(
            ...
            p_trgt       = np.array([0.3,0.2,0.3]) + np.random.uniform([-0.02,-0.02,0], [0.02,0.02,0]),
        ...
        self.obj_init_pose = np.concatenate([cube_init_pose, plate_init_pose],dtype=np.float32)
        for _ in range(100):
            self.step_env()
    ...
    def set_instruction(self, given = None):
        ...
        if given is None:
            # 目前只有一个方块，任务比较固定
            self.instruction = 'Pick up the red cube and move it.'
        else:
            self.instruction = given
    ...
    def check_success(self):
        ...
        if np.linalg.norm(p_cube[:2] - p_plate[:2]) < 0.1 and np.linalg.norm(p_cube[2] - p_plate[2]) < 0.6 and is_gripper_open:
    ...
    def get_obj_pose(self):
        ...
        '''
        p_cube = self.env.get_p_body('cube_main')
        p_plate = self.env.get_p_body('body_obj_plate_11')

        return p_cube, p_plate
    
    def set_obj_pose(self, p_cube, p_plate):
        '''
        ...
        args:
            ...
            p_plate: np.array, position of the plate
        '''
        self.env.set_p_base_body(body_name='cube_main',p=p_cube)
