...
class MyEnv:
    ...
    def reset(self, seed = None):
        ...
        while not valid:
            ...
            dist0 = np.linalg.norm(
                both_xyzs[0, :2] - plate_xyz[:2]
            )

            dist1 = np.linalg.norm(
                both_xyzs[1, :2] - plate_xyz[:2]
            )

            if dist0 > 0.15 and dist1 > 0.15:
                valid = True
        ...
        # 让机械臂初始在物体上方位置
        all_objects = np.vstack([plate_xyz, both_xyzs[0], both_xyzs[1]])
        center_x = np.mean(all_objects[:, 0])
        center_y = np.mean(all_objects[:, 1])
        init_pos = np.array([center_x, center_y, 0.7])
        q_init = np.deg2rad([0, -30, 0, -90, 0, 60, 0])
    ...
    def set_instruction(self, given = None):
        ...
        if given is None:
            ...
            templates = [
                ...
                "Pick up the {} cube.",
                "Move the {} block to the plate.",
                "Grab the {} cube and place it.",
                "Put the {} object on the plate."
                ]

            self.instruction = random.choice(templates).format(obj1)
    ...
    def step(self, action):
        ...
        if self.action_type == 'eef_pose':
            ...
            # 获取当前的eef的位置和方向
            current_p, current_R = self.env.get_pR_body(
                body_name='gripper0_right_eef'
            )
            # 获得目标方向和角度
            target_p = current_p + action[:3]
            print(f"target_p is {target_p}")
            target_R = rpy2r(np.array([np.pi, 0, np.pi/2]))
            # 计算IK，得到每一个joints的目标角度q
        ...
        if not hasattr(self, "gripper_opening"):
            self.gripper_opening = 0.04

        speed = 0.003

        if action[-1] == -1:
            self.gripper_opening += speed
        else:
            self.gripper_opening -= speed

        self.gripper_opening = np.clip(
            self.gripper_opening,
            0.0,
            0.04
        )

        gripper_cmd[0] = self.gripper_opening
        gripper_cmd[1] = -self.gripper_opening
        self.compute_q = q
