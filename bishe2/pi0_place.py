...
# %%
try:
    dataset_metadata = LeRobotDatasetMetadata("my_pi04", root='./demo_data_language4')
except:
    dataset_metadata = LeRobotDatasetMetadata("my_pi04", root='./demo_data_language4')
...
policy = PI0Policy.from_pretrained('./ckpt/my_pi04/checkpoints/last/pretrained_model', dataset_stats=dataset_metadata.stats)
...
PnPEnv = MyEnv(xml_path, action_type='joint_angle')
...
while PnPEnv.env.is_viewer_alive():
    ...
    if PnPEnv.env.loop_every(HZ=20):
        ...
        # PnPEnv.render()
        frame = PnPEnv.env.get_fixed_cam_rgb(cam_name='robotview')
        frames.append(frame)
        step += 1
        success = PnPEnv.check_success()
        if success:
