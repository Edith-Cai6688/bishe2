...
while myenv.env.is_viewer_alive() and episode_id < NUM_DEMO:
    ...
    if myenv.env.loop_every(HZ=20):
        ...
        if stage == 0 and np.linalg.norm(target_pose - eef_pose[:3]) < 0.02:
            stage = 1
            stage_steps = 0
        elif stage == 1 and np.linalg.norm(target_pose - eef_pose[:3]) < 0.02:
            stage = 2
            stage_steps = 0
        elif stage == 2 and  grasp_counter > 10:
            stage = 3
            stage_steps = 0
            grasp_counter = 0
        elif stage == 3 and np.linalg.norm(target_pose - eef_pose[:3]) < 0.02:
            stage = 4
            stage_steps = 0
        elif stage == 4 and np.linalg.norm(target_pose - eef_pose[:3]) < 0.02:
            stage = 5
            stage_steps = 0
        elif stage == 5 and np.linalg.norm(target_pose - eef_pose[:3]) < 0.02:
            stage = 6
            stage_steps = 0
        elif stage == 6 and release_counter > 10:
            stage = 7
            stage_steps = 0
            release_counter = 0
