...
REPO_NAME = 'my_pi0'
NUM_DEMO = 100 # Number of demonstrations to collect
ROOT = "./demo_data_language5_100" # The root directory to save the demonstrations
    ...
    if myenv.env.loop_every(HZ=20):
        ...
        if done:
            ...
            if len(dataset.episode_buffer) == 0:
                print("⚠️ Empty episode, skip saving")
                continue
        ...
        # Move to the cube
        if stage == 0:
            target_pose = cube_pose + np.array([0, 0, 0.3])
        ...
        # Go down
            ...
            target_pose = cube_pose + np.array([0, 0, 0.01])
            # action[:3] = target_pose
            diff_pos = target_pose - eef_pose[:3]
            dist = np.linalg.norm(diff_pos)
            if min(dist, max_dist) == max_dist:
        ...
        # Move to the plate
            ...
            target_pose = plate_pose + np.array([0, 0, 0.2])
            # action[:3] = target_pose
            diff_pos = target_pose - eef_pose[:3]
            dist = np.linalg.norm(diff_pos)
            if min(dist, max_dist) == max_dist:
        ...
        # Go down
            ...
            target_pose = plate_pose + np.array([0, 0, 0.04])
            # action[:3] = target_pose
            diff_pos = target_pose - eef_pose[:3]
            dist = np.linalg.norm(diff_pos)
            if min(dist, max_dist) == max_dist:
        ...
        # Go up
            ...
            target_pose = plate_pose + np.array([0, 0, 0.3])
            # action[:3] = target_pose
            diff_pos = target_pose - eef_pose[:3]
            dist = np.linalg.norm(diff_pos)
            if min(dist, max_dist) == max_dist:
