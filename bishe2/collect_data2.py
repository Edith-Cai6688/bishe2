...
while myenv.env.is_viewer_alive() and episode_id < NUM_DEMO:
    ...
    if myenv.env.loop_every(HZ=20):
        ...
        action, reset  = myenv.teleop_robot()
        ...
        joint_q = myenv.step(action)
        action = np.concatenate([myenv.q[:7], [action[-1]]]) # 6 joint angles and 1 gripper
